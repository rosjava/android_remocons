...
public class YujinNfcManager {
	...
	public YujinNfcManager(Context context) {
	...
	public String ndefMessageToString(NdefMessage message)  {
    	...
    	for(int i = 0 ; i < ndef_records.length ; i++){
    		...
    		temp = temp + "- TNF=" + ndef_records[i].getTnf() + 
    				"\n - TYPE=" + YujinUtil.getHexString(type, type.length) + " " + new String(type) +
    				"\n - ID=" + YujinUtil.getHexString(id, id.length) + " " + new String(id) +
    				"\n - PayLoad=" + YujinUtil.getHexString(pl, pl.length) + " " + new String(pl) +
    				"\n - ByteArray=" + YujinUtil.getHexString(arr, arr.length) + " " + new String(arr) + "\n";
	...
	private NdefRecord createTextRecord(String text, Locale locale, boolean encodeInUtf8) {
    	...
    	final byte[] data = YujinUtil.concat(new byte[] {(byte)status}, langBytes, textBytes);
